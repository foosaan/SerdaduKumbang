...
@section('breadcrumb-sub', 'Perbarui data pengurus')
        ...
        <div class="card-body p-4">
            <form action="{{ route('admin.pengurus.update', $pengurus->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="nama" class="form-control rounded-3 @error('nama') is-invalid @enderror" value="{{ old('nama', $pengurus->nama) }}" required>
                    @error('nama')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label fw-bold">Jabatan <span class="text-danger">*</span></label>
                    <input type="text" name="jabatan" class="form-control rounded-3 @error('jabatan') is-invalid @enderror" value="{{ old('jabatan', $pengurus->jabatan) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <select name="kategori" class="form-select rounded-3 @error('kategori') is-invalid @enderror" required>
                        ...
                        <option value="BPH" {{ old('kategori', $pengurus->kategori) == 'BPH' ? 'selected' : '' }}>Badan Pengurus Harian (BPH)</option>
                        <option value="Staff & Departemen" {{ old('kategori', $pengurus->kategori) == 'Staff & Departemen' ? 'selected' : '' }}>Staff & Departemen</option>
                    </select>
                    @error('kategori')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="mb-3">
                    <label class="form-label fw-bold">Deskripsi Pendek (Opsional)</label>
                    <textarea name="deskripsi" rows="2" class="form-control rounded-3 @error('deskripsi') is-invalid @enderror">{{ old('deskripsi', $pengurus->deskripsi) }}</textarea>
                ...
                <div class="mb-4">
                    ...
                    <div class="mt-3 d-flex align-items-center gap-3">
                        @if($pengurus->foto)
                        <!-- Foto Lama -->
                        <div id="oldPhotoBox" class="text-center border rounded p-2 bg-light w-auto">
                            <img src="{{ asset('storage/' . $pengurus->foto) }}" class="rounded-circle shadow-sm" style="width: 70px; height: 70px; object-fit: cover;">
                            <small class="d-block text-muted mt-2 fw-medium">Foto Lama</small>
                        </div>
                        @endif

                        <!-- Preview Foto Baru -->
                        <div id="croppedPreviewContainer" class="text-center border rounded p-2 bg-white border-primary w-auto d-none">
                            <img id="croppedPreview" src="" class="rounded-circle shadow-sm" style="width: 70px; height: 70px; object-fit: cover;">
                            <small class="d-block text-primary mt-2 fw-bold">Perubahan Baru</small>
                        </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const oldPhotoBox = document.getElementById('oldPhotoBox');
    ...
    btnCrop.addEventListener('click', function() {
        ...
        cropper.getCroppedCanvas({
            ...
            croppedPreviewContainer.classList.add('d-block');
            
            if (oldPhotoBox) {
                oldPhotoBox.style.opacity = '0.4'; // bikin blur foto lama
            }
